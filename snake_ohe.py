...
class SnakeEnv:
    def __init__(self, start_loc, start_dir, width=60, height=60):
        ...
        self.state = np.zeros((width, height, 4), int)
        self.food = [0, 0]
        ...
        self.state[start_loc[0], start_loc[1]] = [0, 0, 1, 0]
    ...
    def update_state(self):
        self.state = np.zeros((self.width, self.height, 4))
        s_body = self.s_body
        for i in s_body[:-1]:
            self.state[i[0], i[1]] = [0, 1, 0, 0]
        self.state[s_body[-1][0], s_body[-1][1]] = [0, 0, 1, 0]
        self.state[self.food[0], self.food[1]] = [0, 0, 0, 1]
